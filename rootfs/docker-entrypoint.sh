...
echo "# Generated by $0 on $(date)" > /etc/apache2/rewrite-env-rules.conf
...
printenv | grep '^REWRITE_' | sort -fibn -t'_' -k2 | while read -r rule; do
    ...
    echo "[$(date +'%F %T.%s' | cut -c-26)] -:PROXY:${ruleid} $src -> $dst"
...
hostdockerinternal_ip=$(getent hosts host.docker.internal | awk '{print $1}')
if [ -n "$hostdockerinternal_ip" ]; then
    export TRUSTED_PROXIES="$TRUSTED_PROXIES $hostdockerinternal_ip/32"
fi

echo "[$(date +'%F %T.%s' | cut -c-26)] -:conf:notice -> Set trusted proxies -- $TRUSTED_PROXIES"
